%%% File name:   UltLimitCoh_DiscPureMPS_approx_RhoRatioScen_A2zeroA0cnstA13sym.m (a Matlab function)
...
function [coh] = UltLimitCoh_DiscPureMPS_approx_RhoRatioScen_A2zeroA0cnstA13sym(D_min, DeltaD, D_max, coefficient0, D_rescale, CalculateCohCorrs)
    ...
    fprintf('EXAMPLE USAGE: run ./UltLimitCoh_DiscPureMPS_approx_RhoRatioScen_A2zeroA0cnstA13sym(50,1,50,0.01,9999,''no'') in a Matlab command-line environment.\n');
    ...
    FilenameFinal = strcat('Coherence_D-RhoRatioScen_A2zeroA0cnstA13sym-UltQuantumLimitProj.out');
    ...
    for D = D_min : DeltaD : D_max
        ...
        filename_opt = strcat('OptimizedMatrices-D',num2str(D),'-RhoRatioScen_A2zeroA0cnstA13sym-UltQuantumLimitProj.out');          
        ...
        if D == D_rescale
            filename_rescale = strcat('CoherenceEps0-AmatricesRescaling-D',num2str(D),'-RhoRatioScen_A2zeroA0cnstA13sym-UltQuantumLimitProj.out');          
        ...
        if strcmp(CalculateCohCorrs,'yes')
            filename_corrs = strcat('CohCorrs_vs_r-D',num2str(D),'-RhoRatioScen_A2zeroA0cnstA13sym-UltQuantumLimitProj.out');          
...
%         end
        ...
        F = @(x) -coh_RhoRatioScen_A2zeroA0cnstA13sym(D,x);

        [x,Fval] = fmincon(F,a_array0,[],[],[],[],Lb,Ub,[],options);
        ...
        [A0,A1,A2,A3] = Amatrices_RhoRatioScen_A2zeroA0cnstA13sym(D,x);
        [~,Ceq,~,~] = inject_func_RhoRatioScen_A2zeroA0cnstA13sym(D,x);
        ...
        if D == D_rescale
          ...
          %for ii = 0:211 
              ...
              [~,Ceq_new,~,~] = inject_func_RhoRatioScen_A2zeroA0cnstA13sym(D,a_array_new);
