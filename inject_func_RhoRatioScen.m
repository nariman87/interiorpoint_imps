...
%  IdentityExpectation = L1_flat0 * T * R1_flat0;
  ...
  %Ceq(1) = sum( sum( Am'*Am + A0'*A0 + Ap'*Ap - I_D ) )/D^2; 
  Ceq(1) = sum(L1_flat0*T - L1_flat0)/D^2;
  %Ceq(1) = abs( L1_flat0 * T * R1_flat0 - 1 );
  Ceq(2) = abs( L1_flat0*(-kron(conj(Am),Am) + kron(conj(Ap),Ap))*R1_flat0 ); % this should be automatically satisfied.
